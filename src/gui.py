...
tab_2 = [
         ...
         [sg.Text('Simple: ')],
         [sg.Checkbox('Firstname', disabled=False, enable_events=True, k=('man_simple_firstname','advanced_options')), \
             sg.Push(),
             sg.Text('Begins with: '),sg.InputText(size=(12,1), disabled=False, enable_events=True, k=('simple_firstname_beginning_input','advanced_options')), \
             sg.Text('Ends with: '),  sg.InputText(size=(12,1), disabled=False, enable_events=True, k=('simple_firstname_ending_input','advanced_options'))],
         [sg.Checkbox('Surname', disabled=False, enable_events=True, k=('man_simple_surname','advanced_options')),\
             sg.Push(),
             sg.Text('Begins with: '), sg.InputText(size=(12,1), disabled=False, enable_events=True, k=('simple_surname_beginning_input','advanced_options')), \
             sg.Text('Ends with: '),   sg.InputText(size=(12,1), disabled=False, enable_events=True, k=('simple_surname_ending_input','advanced_options'))],
         [sg.Text(' ', font=('Helvetica', 15))],
         [sg.Text('Manual: ')],
         #[sg.Text(' ', font=('Helvetica', 15))],
         [sg.Checkbox('Firstname', disabled=False, enable_events=True, k=('man_firstname','advanced_options')), \
             sg.Push(),
             sg.Text('Value: '), sg.InputText(size=(40,1), disabled=False, enable_events=True, k=('man_firstname_input','advanced_options'))],\
         [sg.Checkbox('Surname', disabled=False, enable_events=True, k=('man_surname','advanced_options')),\
             sg.Push(),
             sg.Text('Value: '), sg.InputText(size=(40,1), disabled=False, enable_events=True, k=('man_surname_input','advanced_options'))],\
        ]
...
def create_window():
    while True:
        ...
        # naming options tab
        if values[('man_firstname','advanced_options')] or values[('man_surname','advanced_options')]:
            window[('firstname')].update(True)
            window[('surname')].update(True)
            window[("ffxiv_lala", 'style')].update(disabled= True)
        ...
        elif values[('man_simple_firstname','advanced_options')] or values[('man_simple_surname','advanced_options')]:
            window[('surname')].update(True)
        ...
        if values['character_class'] and values['character_class_combo'] == 'Black Desert':
        ...
        if event == 'generate':
            ...
            if len(style) == 2 and style[0][0] != 'simple':
                ...
                name_style = [item for item in activated_options if item[1] == 'style'][0][0]
            else:
            ...
            if generate_amount:
                for i in range(generate_amount):
                    name = names.generate_name(
                        ...
                        first_begin=values[('simple_firstname_beginning_input','advanced_options')],
                        first_end=values[('simple_firstname_ending_input','advanced_options')],
                        sur_begin=values[('simple_surname_beginning_input','advanced_options')],
                        sur_end=values[('simple_surname_ending_input','advanced_options')],
                        style=name_style,
                        sex=sex)
